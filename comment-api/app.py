...
@app.get("/")
def root():
    return {"message": "Schmoozzer comment API", "endpoints": {"/health": "Health check", "/generate-comment": "Generate LinkedIn comment"}}
...
@app.get("/generate-comment")
def generate_comment_info():
    return {
        "message": "Use POST method to generate comments",
        "method": "POST",
        "endpoint": "/generate-comment",
        "body_schema": {
            "post_text": "string (required) - The text of the LinkedIn post",
            "post_author": "string (required) - Name of the post author",
            "extra_instruction": "string (optional) - Additional instructions for the model"
        },
        "example": {
            "post_text": "Great article on AI",
